...
import { ExtendedToken, TokenError } from '@/types/index'
...
const refreshAccessToken = async (token: ExtendedToken): Promise<ExtendedToken> => {
    try {
        ...
        const { body: refreshedTokens } = await spotifyApi.refreshAccessToken() // gọi body là refreshedTokens
...
const jwtCallback: CallbacksOptions['jwt'] = async ({ token, account, user }) => {
    ...
    if (account && user) {
        extendedToken = {
            ...
            accessTokenExpiresAt: (account.expires_at as number) * 1000 // converted to millisecond
        }
...
export default NextAuth({
    ...
    pages: {
        signIn: '/login' // mình tự design signIn bằng trang /login
    },
    callbacks: {
        jwt: jwtCallback,
